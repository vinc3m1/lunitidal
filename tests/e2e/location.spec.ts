...
test('two-finger tilt is disabled and the map stays flat', async ({ page }) => {
  await page.goto('/');
  await page.getByTestId('expand-map').click();
  await expect(page.getByTestId('map-sheet')).toBeVisible();
  const mapEl = page.locator('[data-testid="map-sheet"] .map');
  await expect(mapEl).toBeVisible();

  const state = await mapEl.evaluate((el) => {
    const m = (el as { _maplibreMap?: any })._maplibreMap;
    if (!m) return null;
    // Even if a gesture somehow fired, the pitch handler must reject it.
    m.setPitch(40);
    return {
      touchPitchEnabled: m.touchPitch.isEnabled(),
      pitchAfterSet: m.getPitch(),
    };
  });

  expect(state).not.toBeNull();
  // The two-finger pitch gesture is turned off...
  expect(state!.touchPitchEnabled).toBe(false);
  // ...but programmatic pitch still works, proving we disabled the gesture and not
  // pitch itself (so future 3D features remain possible).
  expect(state!.pitchAfterSet).toBeCloseTo(40, 0);
});
